...
abstract class Index(val binningMethod: BinningMethod, val cache: SpectrumCache) extends LazyLogging {
  ...
  /**
    ...
    * @return
    */
  final def search(spectrum: SimpleSpectrum, similarity: Similarity, threshold: Double = 0.5, precursorToleranceDa: Double = 0.10, removePrecursorIon: Boolean, calculateAllAdducts: Boolean): Iterable[ComputationalResult] = {

    Calculate.create.calculate(spectrum, this, threshold, similarity, precursorToleranceDa, removePrecursorIon, calculateAllAdducts)
  }
