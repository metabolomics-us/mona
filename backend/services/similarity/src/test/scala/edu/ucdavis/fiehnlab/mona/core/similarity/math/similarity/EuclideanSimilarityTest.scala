...
/**
  ...
  */
class EuclideanSimilarityTest extends AnyFlatSpec with Matchers {
  val EPSILON: Double = 0.0001


  "Euclidean Similarity" should "be unity for identical spectra with one ion" in {
    // MassBank EA034401
    val s: SimpleSpectrum = new SimpleSpectrum("", "169.9232:3169812.7")
    s.ions.length shouldEqual 1

    val similarity = new EuclideanSimilarity().compute(s, s)
  ...
  it should "compute correctly for different spectra" in {
    ...
    val similarity = new EuclideanSimilarity().compute(unknown, library)
    similarity shouldBe 0.9976 +- EPSILON
  }
}
