  ...
  "a webhook controller" must {
    ...
    "be able to synchronize itself against a main mona server" should {
      ...
      "able to handle add events " in {
        ...
        //add mona spectrum
        given().body(getValue).when().get(s"/webhooks/sync/HMDB00001_1/add").then().statusCode(200)
      ...
      "able to handle update events " in {
        ...
        //add mona spectrum
        given().body(getValue).when().get(s"/webhooks/sync/HMDB00001_1/update").then().statusCode(200)
      ...
      "throw an exception for wrongly specified events" in {
        given().body(getValue).when().get(s"/webhooks/sync/HMDB00001_1/thisIsNoEvent").then().statusCode(400)
      }
    }
  }
