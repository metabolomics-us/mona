...
class WebhookControllerTest extends AbstractGenericRESTControllerTest[WebHook]("/webhooks") with Eventually with Matchers with LazyLogging {
  ...
  /**
    ...
    * @return
    */
  override def getValue: WebHook = WebHook("testWebHook", s"http://localhost:$port/info", "just for testing", "test")
  ...
  "a webhook controller" must {
    ...
    "secure endpoints" should {
      ...
      "a user is able to create a webhook" in {
        ...
        assert(result.name == "testWebHook")
    ...
    "be able to trigger the registered external urls and not crash in case they cant be reached" in {
      webHookRepository.deleteAll()
      authenticate().contentType("application/json; charset=UTF-8").body(WebHook("i can't be reached", "http://localhost:21234/rest", "none provided", "test")).when().post(s"/webhooks").`then`().statusCode(200)
    ...
    "be able to trigger the registered external urls and not crash in case they don't exist" in {
      webHookRepository.deleteAll()
      authenticate().contentType("application/json; charset=UTF-8").body(WebHook("i dont exist", s"http://localhost:$port/info/id/", "this tosses a 401", "test")).when().post(s"/webhooks").`then`().statusCode(200)
