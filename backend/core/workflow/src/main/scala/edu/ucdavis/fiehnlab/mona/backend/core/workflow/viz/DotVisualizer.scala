...
import edu.ucdavis.fiehnlab.mona.backend.core.workflow.graph.{AbstractEdge, AbstractVertex, Edge, Graph}
...
class DotVisualizer {
  ...
  def visualize[ID](graph:Graph[ ID, _ <: AbstractVertex[ID], _ <: AbstractEdge[ID]], outputStream: OutputStream) ={
