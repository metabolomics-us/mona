...
class CalculateCompoundProperties extends ItemProcessor[Spectrum, Spectrum] with LazyLogging {
  ...
  override def process(spectrum: Spectrum): Spectrum = {
    ...
    val updatedCompound: Buffer[CompoundDAO] = spectrum.getCompound.asScala.map(compound => calculateCompoundProperties(compound, spectrum.getId, impacts))
  ...
  def calculateCompoundProperties(compound: CompoundDAO, id: String, impacts: ArrayBuffer[Impacts]): CompoundDAO = {
    logger.info(s"$id: Processing compound: ${compound.getKind}")

    // Updated metadata to add to this compound
    val metaData: ArrayBuffer[MetaDataDAO] = new ArrayBuffer[MetaDataDAO]()
    ...
    if (compound.getInchiKey != null && !compound.getInchiKey.isEmpty && compound.getMetaData.asScala.forall(_.getName.toLowerCase != CommonMetaData.INCHI_KEY.toLowerCase)) {
      metaData.append(new MetaDataDAO(null, CommonMetaData.INCHI_KEY, compound.getInchiKey, false, "none", false, null))
    }
    ...
    if (molecule == null) {
      ...
      metaData.append(new MetaDataDAO(null, CommonMetaData.MOLECULAR_FORMULA, compoundConversion.moleculeToMolecularFormula(molecule), false, "computed", true, null))

      metaData.append(new MetaDataDAO(null, CommonMetaData.TOTAL_EXACT_MASS, compoundConversion.moleculeToTotalExactMass(molecule).toString, false, "computed", true, null))

      // Calculate SMILES
      metaData.append(new MetaDataDAO(null, CommonMetaData.SMILES, compoundConversion.moleculeToSMILES(molecule), false, "computed", true, null))
      ...
      val providedInChI: Option[MetaDataDAO] = (compound.getMetaData.asScala ++ metaData)
        .find(x => x.getName.toLowerCase == CommonMetaData.INCHI_CODE.toLowerCase && !x.getComputed)
      val providedInChIKey: Option[MetaDataDAO] = (compound.getMetaData.asScala ++ metaData)
      ...
      if (providedInChI.isEmpty || providedInChI.get.getValue.toString != computedInChI) {
        metaData.append(new MetaDataDAO(null, CommonMetaData.INCHI_CODE, computedInChI, false, "computed", true, null))
      }

      if (providedInChIKey.isEmpty || providedInChIKey.get.getValue.toString != computedInChIKey) {
        metaData.append(new MetaDataDAO(null, CommonMetaData.INCHI_KEY, computedInChIKey, false, "computed", true, null))
      }
