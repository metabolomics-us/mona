...
### Association of compounds with spectra

Due to the nature of MoNa, it is possible that one compound, can be associated with several spetra. This histogram, provides you with a quick overview of this data

```{r,include=FALSE}

monaCompoundSplashes = sqldf("
          select count(*), a.inchi_key , splash from compound a, spectrum b, compound_link c, splash d where c.type = 'BIOLOGICAL' and b.id = d.spectrum_id and a.id = c.compound_id and b.id = c.spectrum_id group by inchi_key, splash, inchi_key || splash order by count desc
...
```{r,echo=FALSE}
set.caption("compounds with most associated spectra")

panderOptions("digits", 1)
pander(head(monaCompoundSplashes,10), split.table = Inf)
```
...
ggplot(data=experimentalVsPredictedSpectraReport,x=experimentalVsPredictedSpectraReport$database,y=experimentalVsPredictedSpectraReport$count,geom="bar",stat="identity",xlab="type of spectra",ylab="count of spectra") + theme_bw()+ theme(axis.text.x = element_text(angle = 90, hjust = 1))+ coord_flip()
```
...
ggplot(data=spectraCountForDatabases,x=spectraCountForDatabases$database,y=spectraCountForDatabases$count,geom="bar",stat="identity",xlab="origin",ylab="count of spectra") + theme_bw()+ theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
...
ggplot(data=duplicatedSpectraReport,x=c('total','uniquespectra','duplicated'),y=c(duplicatedSpectraReport$total,duplicatedSpectraReport$uniquespectra,duplicatedSpectraReport$duplicated),geom="bar",stat="identity",xlab="spectra",ylab="count of spectra") + theme_bw()+ theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  coord_flip()
...
```{r,echo=FALSE,fig.width=7, fig.height=4, fig.align='center'}
ggplot(data=aquistionMethodReport,x=aquistionMethodReport$method,y=aquistionMethodReport$count,geom="bar",stat="identity",xlab="aquisition method",ylab="count of spectra") + theme_bw()+ theme(axis.text.x = element_text(angle = 90, hjust = 1))+ coord_flip()
```
...
```{r,echo=FALSE,fig.width=7, fig.height=4, fig.align='center'}
ggplot(data=msLevelTypeReport,x=msLevelTypeReport$value,y=msLevelTypeReport$count,geom="bar",stat="identity",xlab="MS Type",ylab="count of spectra") + theme_bw()+ theme(axis.text.x = element_text(angle = 90, hjust = 1)) + coord_flip()
```
...
```{r,echo=FALSE,fig.width=7, fig.height=4, fig.align='center'}
ggplot(data=ionModeReport,x=ionModeReport$value,y=ionModeReport$count,geom="bar",stat="identity",xlab="Ion Mode",ylab="count of spectra") + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + coord_flip()
```
...
ggplot(data=precursorTypeReport,x=precursorTypeReport$value,y=precursorTypeReport$count,geom="bar",stat="identity",xlab="Precursor Type",ylab="count of spectra") + theme_bw()+ theme(axis.text.x = element_text(angle = 90, hjust = 1)) + coord_flip()
```
...
```{r,echo=FALSE,fig.width=7, fig.height=8, fig.align='center'}
ggplot(data=derivativeTypeReport,x=derivativeTypeReport$value,y=derivativeTypeReport$count,geom="bar",stat="identity",xlab="Derivative Type",ylab="count of spectra") + theme_bw()+ theme(axis.text.x = element_text(angle = 90, hjust = 1)) + coord_flip()
```
...
```{r,echo=FALSE,fig.width=7, fig.height=8, fig.align='center'}
ggplot(data=instrumentTypeReport,x=instrumentTypeReport$value,y=instrumentTypeReport$count,geom="bar",stat="identity",xlab="Instrument Type",ylab="count of spectra") + theme_bw()+ theme(axis.text.x = element_text(angle = 90, hjust = 1)) + coord_flip()
```
...
```{r,echo=FALSE,fig.width=7, fig.height=8, fig.align='center'}
ggplot(data=tagsDistribtution,x=tagsDistribtution$value,y=tagsDistribtution$count,geom="bar",stat="identity",xlab="Tag/Label",ylab="count of spectra") + theme_bw()+ theme(axis.text.x = element_text(angle = 90, hjust = 1)) + coord_flip()
```
...
## origins of visitors for the current month

```{r,include=FALSE}

currentOrigins = tables[[21]]
currentOrigins = currentOrigins[-c(1),c(2,4,5,6)]

colnames(currentOrigins)[1] = "Country"
colnames(currentOrigins)[2] = "Pages"
colnames(currentOrigins)[3] = "Hits"
colnames(currentOrigins)[4] = "Bandwitdh"
...
```{r,echo=FALSE,fig.width=7,fig.height=3,fig.align='center'}
panderOptions("digits", 0)
pander(currentOrigins[c(1,2,3,4)], split.table = Inf)

```

## registered submitters

This graph illustrates how the registered userbase of MoNa is growing over the last 12 Months.

```{r,include=FALSE}

submitterCount = sqldf("select count(*), date_trunc('month',date_created) as date from submitter group by date order by date asc")

```

```{r,echo=FALSE,fig.width=7,fig.height=3,fig.align='center'}

ggplot(data=submitterCount,aes(x= date,y=count)) + geom_bar(stat = "identity", position="dodge") + xlab("month") + ylab("registered user count")+ theme_bw() + ggtitle("registered users by month") + geom_line(aes( y = cumsum(count)), size = 1, color = 'darkgray')
```

## generated compounds

This graph illustrates how many compounds have been generated in the system over the last 12 Months


```{r,include=FALSE}

compoundCount = sqldf("
select count(*), date_trunc('day',date_created) as date from compound a, supports_meta_data b where a.id = b.id group by date order by date asc
                     ")
...
ggplot(data=compoundCount,aes(x= date,y=count)) + geom_bar(stat = "identity", position="dodge") + xlab("date") + ylab("uploaded compound count")+ theme_bw() + ggtitle("uploaded compounds by month") + geom_line(aes( y = cumsum(count)), size = 1, color = 'darkgray')
```


## uploaded spectra

This graph illustrates how many spectra have been imported in the system over the last 12 Months


```{r,include=FALSE}

spectraCount = sqldf("
                     select count(*), date_trunc('day',date_created) as date from spectrum a, supports_meta_data b where a.id = b.id group by date order by date asc
                     ")
...
ggplot(data=spectraCount,aes(x= date,y=count)) + geom_bar(stat = "identity", position="dodge") + xlab("date") + ylab("uploaded spectra count")+ theme_bw() + ggtitle("uploaded spectra by month") + geom_line(aes( y = cumsum(count)), size = 1, color = 'darkgray')
```
...
ggplot(data=split(metaDataTypeReport,1:2)[[1]],x=name,y=count,geom="bar",stat="identity",xlab="Meta Data Type",ylab="count of values", fill=type)+ theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + coord_flip()

```

```{r,echo=FALSE,fig.width=8, fig.height=10, fig.align='center'}

ggplot(data=split(metaDataTypeReport,1:2)[[2]],x=name,y=count,geom="bar",stat="identity",xlab="Meta Data Type",ylab="count of values", fill=type)+ theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + coord_flip()+ theme(axis.ticks = element_blank())
