...
qplot(c('accepted','deleted because of duplication'),num,data=deletedSpectraReport,geom = "bar", stat = "identity", ylab = "count of spectra", xlab = "status of spectra")+ theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  coord_flip()
...
Current estimates, based on the observed performance allow us to predict that the system can import $`24*60*60*1000/mean(importStatistics$value)`$ spectra in a single day.

## Validation Performance

## Scoring Perfomance

Once the import is complete, the system automatically scores the received spectra, based on it's metadata and the correctness of these data, as well as there association with other metdata.
...
Current estimates, based on the observed performance allow us to predict that the system can score $`24*60*60*1000/mean(scoringStatistics$value)`$ spectra in a single day.
