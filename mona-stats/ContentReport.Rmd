...
histogramDistribution = sqldf("
    select count(*)::integer, block4::text from splash group by block4 having count(*) > (select count(*) * 0.005 from splash)
")
...
metaDataTypeReportSegement1 = split(metaDataTypeReport, sample(rep(1:2,length(metaDataTypeReport)/2)))[[1]]
metaDataTypeReportSegement2 = split(metaDataTypeReport, sample(rep(1:2,length(metaDataTypeReport)/2)))[[2]]
...
qplot(data=metaDataTypeReportSegement1,x=name,y=count,geom="bar",stat="identity",xlab="Meta Data Type",ylab="count of values", fill=type)+ theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + coord_flip()
...
qplot(data=metaDataTypeReportSegement2,x=name,y=count,geom="bar",stat="identity",xlab="Meta Data Type",ylab="count of values", fill=type)+ theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + coord_flip()

```

## Similar named meta data names

This provides us with a quick overview of possible duplicated meta data names, which allows us to adjust the rules and filter these values during the next validation cycle.
...
select a.id, a.name as a,b.name as b,similarity(a.name,b.name) as sim from meta_data a, meta_data b, meta_data_category c where a.category_id = c.id and b.category_id = c.id and lower(c.name) not in ('annotation','external id') and a.id != b.id and similarity(a.name,b.name) > 0.05 order by sim desc
) a,
...
```{r,echo=FALSE,fig.width=8,fig.height=8,fig.align='center'}
ggplot(duplicatedMetaDataReportSplit[[4]], aes(a,b)) + theme_bw() + geom_tile(aes(fill = sim), colour = "white") + scale_fill_gradient(low = "white", high = "steelblue", name = "similarity") + xlab("meta data name") + ylab("meta data name") + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + scale_x_discrete(expand = c(0, 0)) + scale_y_discrete(expand = c(0, 0)) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + theme(axis.ticks = element_blank())
```
