    ...
    protected boolean acceptMetaDataValue(MetaDataValue value) {
        ...
        if(!hasIonModeSpecified) {
            ...
            if (value.getValue().toString().trim().endsWith("+")) {
                metaDataPersistenceService.generateMetaDataObject(value.owner,[name:ION_MODE,value:"positive"])
            }
            //negative mode
            else if (value.getValue().toString().trim().endsWith("-")) {
                metaDataPersistenceService.generateMetaDataObject(value.owner,[name:ION_MODE,value:"negative"])
            }
        }
        else{
